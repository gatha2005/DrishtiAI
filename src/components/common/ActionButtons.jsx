...
// ── Convert coordinates → spatial descriptions ────────────
...
// ── Sketch hash — same sketch+attrs = same hash = cached ──
...
export default function ActionButtons({ sketchItems = [] }) {
  ...
  // ── Generate real face via backend ────────────────────────
  const handleGenerate = async () => {
    ...
    try {
      ...
      // 1. Check backend cache — same sketch returns instantly
      ...
      // 3. Send to backend → Replicate img2img
  ...
  const handleSubmit = async () => {
    try {
      ...
      const record = {
        id: `REC-${Date.now()}`,
        subject: `Unknown ${attrs.gender === "man" ? "Male" : attrs.gender === "woman" ? "Female" : "Person"}, ~${attrs.age}yr`,
        caseId, date: new Date().toISOString().slice(0, 10),
        status: "Submitted", sketchUrl: dataUrl,
  ...
  const placedFeatures = [...new Set(sketchItems.map(i => getFolder(i.src)).filter(Boolean))];
    ...
    <>
      ...
      <div style={s.panel}>
        <div style={s.panelHdr} onClick={() => setShowAttrs(v => !v)}>
          ...
          <div style={s.tagRow2}>
            ...
            {attrs.skinTone && <Tag c="#8b6914">SKIN: {attrs.skinTone}</Tag>}
        ...
        {showAttrs && (
          <div style={s.attrGrid}>
            ...
            <div style={s.attrBox}>
              ...
              <input type="range" min={10} max={80} value={attrs.age}
            ...
            <div style={s.attrBox}>
              ...
              <div style={{ display:"flex", gap:"6px" }}>
                {SKIN_TONES.map(({ k, hex }) => (
                  <button key={k} title={k} onClick={() => set("skinTone", k)}
                    style={{ width:26, height:26, borderRadius:"50%", background:hex, cursor:"pointer",
                      border: attrs.skinTone === k ? "3px solid #f5a623" : "2px solid #1e2d3d" }} />
                ))}
              </div>
            </div>
            ...
            <div style={s.attrBox}>
              ...
              <select value={attrs.scars} onChange={e => set("scars", e.target.value)} style={s.sel}>
                ...
                <option value="near right eye">Near Right Eye</option>
            ...
            <div style={s.attrBox}>
              ...
              <select value={attrs.marks} onChange={e => set("marks", e.target.value)} style={s.sel}>
                ...
                <option value="mole on right cheek">Mole — Right Cheek</option>
      ...
      {sketchItems.length > 0 && (
        <div style={s.summaryBar}>
          ...
          {placedFeatures.map(f => <Tag key={f} c="#00e5a0">{f.replace("_"," ").toUpperCase()} ✓</Tag>)}
      ...
      {promptPreview && (
        <div style={s.promptBox}>
          <div style={s.lbl}>GENERATION PROMPT (COORDINATES + FEATURES)</div>
      ...
      {modal === "realface" && (
        <div style={s.overlay} onClick={() => setModal(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.mHdr}>
              <span style={s.mTitle}>
                <Dot c="#a855f7" /> GENERATED SUSPECT FACE
                {result?.fromCache && <span style={s.cacheBadge}>⚡ CACHED</span>}
              </span>
            ...
            <div style={s.imgWrap}>
              {!imgLoaded && !imgError && (
                <div style={s.skeleton}>
                  <span style={s.spinLg} />
                  <div style={s.skelTxt}>GENERATING FACE...</div>
                  <div style={s.skelSub}>Please wait 15–30 seconds</div>
                </div>
              )}
              {imgError && (
                <div style={s.skeleton}>
                  <div style={{ fontSize:"32px" }}>⚠</div>
                  <div style={{ ...s.skelTxt, color:"#ff3b4e" }}>IMAGE FAILED TO LOAD</div>
                  <div style={s.skelSub}>Click REGENERATE</div>
                </div>
              )}
              {result?.imageUrl && (
                <img src={result.imageUrl} alt="Generated face"
                  style={{ ...s.img, display: imgLoaded ? "block" : "none" }}
            ...
            {/* Prompt */}
            {result?.prompt && (
              <div style={{ padding:"10px 16px", borderTop:"1px solid #1e2d3d" }}>
                <div style={s.lbl}>PROMPT (COORDINATES + FEATURES)</div>
                <div style={s.promptTxt}>{result.prompt}</div>
              </div>
            )}

            <div style={s.warning}>⚠ AI-generated for forensic reference only. Not a real photograph.</div>

            <div style={s.mFoot}>
              <button style={{ ...s.btn, ...s.amber, flex:1 }} onClick={() => {
                const a = document.createElement("a");
                a.download = `face_${Date.now()}.png`;
                a.href = result?.imageUrl; a.click();
              }}>↓ SAVE</button>
              <button style={{ ...s.btn, ...s.purple, flex:1 }}
                onClick={() => { setModal(null); setTimeout(handleGenerate, 100); }}>↺ REGENERATE</button>
              <button style={{ ...s.btn, ...s.green, flex:1 }}
                onClick={() => { handleSubmit(); setModal(null); }}>✓ SUBMIT</button>
            </div>
          </div>
        </div>
      )}
...
function Tag({ c, children }) {
  return <span style={{ fontFamily:"var(--font-mono)", fontSize:"9px", padding:"2px 7px", borderRadius:"3px", background:`${c}15`, border:`1px solid ${c}33`, color:c, letterSpacing:"0.5px" }}>{children}</span>;
}
function Dot({ c, sz = 8 }) {
...
function Spin({ color, text }) {
  return (
    <span style={{ display:"flex", alignItems:"center", gap:"7px" }}>
      <span style={{ width:12, height:12, border:`2px solid ${color}44`, borderTop:`2px solid ${color}`, borderRadius:"50%", animation:"spin 0.8s linear infinite", display:"inline-block" }} />
      <span style={{ fontFamily:"var(--font-mono)", fontSize:"10px", letterSpacing:"1px" }}>{text}</span>
    </span>
  );
}
